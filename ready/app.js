...
async function submitOrder(){
  const q=state.questions[state.questionIndex],order=$$('#order-list .order-item').map(item=>item.dataset.chunkId),data=await safely(()=>call('submit_attempt',{examId:state.exam.id,questionId:q.id,order,elapsedMs:Math.round(performance.now()-state.startedAt)},state.token,'답을 저장하는 중…'));if(!data)return;
