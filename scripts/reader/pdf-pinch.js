...
function finishOriginalPinch(){
  ...
  setOriginalZoom(pinch.next,pinch.position);
  clearTimeout(originalPinchWheelTimer);
  originalPinchWheelTimer = 0;
  resharpenOriginalPages();
  saveReadingState();
}
function cancelOriginalPinch(){
  cancelAnimationFrame(originalPinchFrame);
  clearTimeout(originalPinchWheelTimer);
  originalPinchFrame = originalPinchWheelTimer = 0;
  const pinch = originalPinch;
  originalPinch = null;
...
function originalPinchWheel(event){
  if(!event.ctrlKey || originalPinchTouches || !originalPinchTarget(event.target) || !event.cancelable) return;
  event.preventDefault();
  const center = {x:event.clientX,y:event.clientY};
  if(!originalPinch) beginOriginalPinch(center,1,[]);
  const delta = event.deltaY*(event.deltaMode === 1 ? 16 : event.deltaMode === 2 ? readerViewHeight() : 1);
  moveOriginalPinch(originalPinch.next*Math.exp(-delta*.01),center);
  clearTimeout(originalPinchWheelTimer);
  originalPinchWheelTimer = setTimeout(finishOriginalPinch,150);
}
(function(){
  const start = ()=>{
    ...
    box.addEventListener('wheel',originalPinchWheel,{passive:false});
  };
