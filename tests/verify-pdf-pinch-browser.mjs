...
try{
 for(const [engine,width,height] of [[chromium,390,844],[chromium,768,1024],[webkit,390,844]].filter(([engine])=>!process.env.BREEZE_QA_ENGINE||engine.name()===process.env.BREEZE_QA_ENGINE)){
  ...
  try{
   ...
   assert.equal(await page.locator('#pdfzoom-out,#pdfzoom-in,#aa-pdfzoom').count(),0);
   ...
   // Ctrl-wheel is Chromium's trackpad pinch path; ordinary wheel remains scrolling.
   if(cdp){
     await page.mouse.move(center.x,center.y);
     const before=(await snapshot()).zoom;
     await page.keyboard.down('Control');await page.mouse.wheel(0,-45);await page.keyboard.up('Control');
     await page.waitForTimeout(250);
     assert.ok((await snapshot()).zoom>before);
   }
