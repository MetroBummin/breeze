...
이미 저장한 lexical item은 **네트워크 재판정 없이 기기에 저장된 Meaning을 즉시 보여 줍니다.**
새 뜻이 필요할 때만 사용자가 다시 AI lookup을 요청합니다.
...
이미 저장한 lexical item을 다시 선택
  → 저장된 Meaning을 기기에서 즉시 표시
  → 서버 판정 없음
  → 새 뜻이 필요하면 사용자가 다시 찾기를 요청
...
이미 단어장에 있는 lexical item은 새 문장에서 다시 눌러도 서버에 보내지 않습니다.

- 같은 문장에 확정해 둔 Meaning이 있으면 그것을 바로 보여 줍니다.
- 그렇지 않으면 최근에 본 저장 Meaning을 바로 보여 줍니다.
- 저장된 Meaning이 현재 문장과 맞지 않는다고 느낄 때만 사용자가 새 뜻 찾기를 요청합니다.
- 저장된 Meaning을 다시 보여 주는 것 자체에는 AI 비용이나 네트워크 대기가 없습니다.
...
처음 보는 lexical item이나 사용자가 새 뜻 찾기를 요청한 경우에는 낱말, 문장(최대 600자),
token 목록과 클릭 token index가 생성 AI 제공자에게 전달됩니다.
