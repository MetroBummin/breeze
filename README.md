...
- 로그인한 사용자는 동의 후 현재 위치부터 다음 약 8쪽을 AI가 자동 조판합니다. AI는 먼저 문단 경계를 복원하고, 완성된 문단의 마크다운 역할을 따로 판정합니다.
...
첫 번째 AI 단계는 PDF 줄바꿈 때문에 갈라진 문단만 합치고, 두 번째 단계는 모든 완성 문단에 본문·제목·인용문 등의 역할을 붙입니다.
응답에는 책 문장이 아니라 문단 위치·역할·줄바꿈 정보만 포함되며, 원문 배열은 변경하지 않습니다.
여러 문장으로 된 긴 본문을 제목으로 고르는 등 명백한 결과는 서버와 브라우저의 안전 규칙이 본문으로 되돌립니다.
