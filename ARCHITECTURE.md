...
PDF 원본은 두 손가락 핀치로 1–4배 확대합니다. 트랙패드의 Ctrl-wheel 핀치도
같은 경로를 사용합니다. 확대·축소 버튼은 없습니다. EPUB 원본과 글자 모드는 PDF
배율을 쓰지 않고 `Aa` 설정을 사용합니다.

이전 핀치는 iOS 홈 화면 웹앱에서 viewport와 스크롤 앵커가 함께 바뀌며 페이지가
튀어 `58972f0`에서 버튼으로 전환했습니다. 새 구현(`scripts/reader/pdf-pinch.js`)은
시작 때 잡은 PDF 좌표를 유지하고, 손짓 중에는 프레임당 transform 하나만 갱신합니다.
문서 크기와 scrollTop/scrollLeft는 모든 손가락을 뗄 때 한 번 확정합니다.
시작 시 이전 모드 진입의 지연 위치 복원을 무효화해 손짓을 덮어쓰지 못하게 합니다. 브라우저 전체 확대와
확대 중 새 캔버스 렌더는 막고, 종료 후 보이는 쪽을 다시 선명하게 그립니다.

두 손가락 모두 PDF에서 시작해야 하며 Aa·모달·모바일 사전 시트가 열려 있으면
확대를 시작하지 않습니다. 손가락이 하나 남아도 마지막 손가락을 뗄 때까지 같은
손짓으로 처리해 탭과 길게 누르기를 막습니다. 한 손가락 스크롤이 이미 시작됐다면
그 손짓은 브라우저에 맡기고, 손을 뗀 다음 핀치를 시작합니다. 모드 전환·화면 크기
변경·취소·백그라운드 전환은 미리보기와 예약 프레임을 정리합니다.

회귀 검사: `npx playwright install chromium webkit` 후 `npm run test:pdf-pinch`.
Chromium은 브라우저 터치 입력을, WebKit은 합성 DOM 터치 이벤트를 사용합니다. WebKit
검사는 실물 iPhone Safari/PWA/WKWebView의 제스처 판정을 대신하지 않습니다.
