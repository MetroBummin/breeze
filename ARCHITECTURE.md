> **2026-09-25 구현 계약 정정**
> 현재 활성 동기화는 단어장이며 새 서버 단어장 레코드는 종단간 암호화되지 않습니다. 책 원본은 기기에 보관하며 읽기 진행도 동기화는 현재 비활성 상태입니다. 아래 과거 vault/E2EE 설명은 호환·이행 경로이지 현재 단어장 쓰기 계약이 아닙니다.
> Article Preview 기능을 활성화하면 사용자가 선택한 기사의 제목·짧은 발췌·출처 URL을 Preview 서버로 전송하고, 제목·발췌를 OpenRouter 모델로 처리합니다. 생성된 한국어 소개와 출처 URL은 기사별 공유 캐시에 저장될 수 있으며 개인 책 원본 전체를 공유하지 않습니다. 로컬 추천은 읽기 기록을 기기에서 계산하며 추가 추천 서버로 전송하지 않습니다.
