...
// One article by default; --indices=1,8,9 selects one-based corpus entries.
...
const indicesArg = process.argv.find(arg => arg.startsWith('--indices='))?.slice('--indices='.length);
const indices = indicesArg?.split(',').map(Number);
if (indicesArg && (!indices?.length || indices.some(index => !Number.isInteger(index) || index < 1 || index > articles.length)))
  throw new Error('Invalid --indices; use comma-separated one-based corpus indices');
const selected = indices ? indices.map(index => articles[index - 1]) :
  process.argv.includes('--remaining') ? articles.slice(1) :
...
for (const article of selected) {
  try {
    ...
    results.push({ corpusIndex: articles.indexOf(article) + 1, source: article.source, title: article.title, url: article.url,
    ...
    results.push({ corpusIndex: articles.indexOf(article) + 1, source: article.source, title: article.title, url: article.url,
      ...
      candidateMeta: error?.candidate || null });
    console.log(JSON.stringify({ index: results.length, source: article.source, status: 'failed',
      error: results.at(-1).error }));
    if (selected.length === 1) break;
  }
}
const output = indices ? '/tmp/breeze-article-preview-ai-prompt-targeted-20260925.json' :
