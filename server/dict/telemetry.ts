export type AiAction="look"|"retry"|"explain"|"seed"|"judge"|"phrase";
...
export function meteredFetch(client:RpcClient,trace:AiTrace,provider:Provider,model:string,input:string,init:RequestInit):Promise<Response>{const attempt=++trace.attempt;const task=(async()=>{const start=Date.now();let response:Response;try{response=await fetch(input,init)}catch(error){await persist(client,{p_request_id:trace.requestId,p_attempt:attempt,p_action:trace.action,p_provider:provider,p_model:model,p_http_status:null,p_duration_ms:Math.min(3600000,Math.max(0,Date.now()-start)),p_outcome:"network_error",p_usage:null});throw error}let body:any=null;try{body=await response.clone().json()}catch{}await persist(client,{p_request_id:trace.requestId,p_attempt:attempt,p_action:trace.action,p_provider:provider,p_model:model,p_http_status:response.status,p_duration_ms:Math.min(3600000,Math.max(0,Date.now()-start)),p_outcome:response.ok?"api_ok":"api_error",p_usage:tokenUsage(provider,provider==="gemini"?body?.usageMetadata:body?.usage)});return response})();const runtime=(globalThis as any).EdgeRuntime;if(typeof runtime?.waitUntil==="function")runtime.waitUntil(task.then(()=>{},()=>{}));return task}
