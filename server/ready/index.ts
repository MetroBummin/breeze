...
const adminOps = new Set(["teacher_bootstrap", "create_exam", "update_exam_passages", "create_passage", "update_passage", "update_passage_study", "retry_passage_study", "create_passage_batch", "delete_passage", "reorder_passages", "generate_order", "update_question", "set_question_status", "delete_question", "create_student", "set_student_pin", "set_student_active", "delete_student"]);
...
async function generateWithAnthropic(prompt: string, schema = ORDER_SCHEMA) {
...
async function generateWithOpenAI(prompt: string, schema = ORDER_SCHEMA) {
...
async function createPassage(body: any) {
  ...
  const passage = rows<any>(await db.from("ready_passages").insert({ study_set_id: null, exam_id: null, title: required(body.title, "지문 제목", 120), source_text: sourceText, position: 0, display_order: Number(current[0]?.display_order ?? -1) + 1, source_type: sourceType, grade, source_year: sourceYear, source_month: sourceMonth, source_label: clean(body.sourceLabel, 120), study_status: "pending", translation_source: "none" }).select().single());
  const sentenceResult = await db.from("ready_passage_sentences").insert(pieces.map((text, sentence_index) => ({ passage_id: passage.id, sentence_index, text }))).select().order("sentence_index"); const sentences = rows<any[]>(sentenceResult);
  await preparePassageStudy(passage.id, sentences, teacherTranslations(body.translations, sentences.length));
...
async function wordLookup(body: any, session: ReadySession) { const context = await studyContext(body, session), surfaceWord = required(body.word, "단어", 100), normalized = normalizedWord(surfaceWord); if (!normalized) throw new ApiError(400, "영어 단어만 조회할 수 있습니다."); const event = await db.from("ready_word_lookup_events").insert({ student_id: context.student.id, exam_id: context.examId, passage_id: context.passage.id, sentence_id: context.sentence?.id || null, surface_word: surfaceWord, normalized_word: normalized }); if (event.error) throw new ApiError(500, event.error.message); const cached = await db.from("ready_word_cache").select("meaning").eq("normalized_word", normalized).maybeSingle(); if (cached.error) throw new ApiError(500, cached.error.message); if (cached.data?.meaning) return { word: surfaceWord, normalizedWord: normalized, meaning: cached.data.meaning, cached: true }; let meaning = "뜻을 찾지 못했어요."; try { const response = await fetch(`https://translate.googleapis.com/translate_a/single?client=gtx&sl=en&tl=ko&dt=t&q=${encodeURIComponent(normalized)}`), json = await response.json(), value = (json?.[0] || []).map((item:any) => item?.[0]).filter(Boolean).join("").trim(); if (value) meaning = value; } catch { /* a lookup event is still valuable when the public dictionary is unavailable */ }
...
async function dispatch(op: string, body: any, session: ReadySession | null) {
  switch (op) {
    ...
    case "create_exam": return createExam(body); case "update_exam_passages": return updateExamPassages(body); case "create_passage": return createPassage(body); case "update_passage": return updatePassage(body); case "update_passage_study": return updatePassageStudy(body); case "retry_passage_study": return retryPassageStudy(body); case "create_passage_batch": return createPassageBatch(body); case "delete_passage": return deletePassage(body); case "reorder_passages": return reorderPassages(body); case "generate_order": return generateOrderQuestion(body); case "update_question": return updateQuestion(body); case "set_question_status": return setQuestionStatus(body); case "delete_question": return deleteQuestion(body);
